...
describe("Test stream proto schema", () => {
    ...
    it("Should encode and decode blocks", () => {
        const blockObject = {
            ...
            expect(BlockHeader.encode(decodedBlockHeader).finish().toString()).toBe(encoded.toString())
            ...
            expect(serializer.serialize(header_decoded).toString('hex')).toBe(ser_enc.toString('hex'))
    })
})